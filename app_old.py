...
def truncate_stream(s, max_notes):
    ...
    if isinstance(s, stream.Score):
        ...
        result = _truncate_part(s)
        result.insert(0, key.Key('C'))
        result.insert(0, metadata.Metadata())
...
def generate_midi():
    ...
    # applica troncamento se richiesto
    max_notes = int(data.get("max_notes", 0)) or None
    if max_notes:
        s = truncate_stream(s, max_notes)
